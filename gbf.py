...
def random_click(ele, variance):
    clicked = False
    while not clicked:
        try:
            var_x = randint(0 - variance, variance)
            var_y = randint(0 - variance, variance)
            log("%d, %d" %(ele.location["x"], ele.location["y"]))
            log("Clicking %s with variance X: %d, Y: %d" %(ele, var_x, var_y))
            actions = ActionChains(GBF)
            actions.move_to_element(ele).click().perform()
            clicked = True
        except StaleElementReferenceException:
            log("Element not attached to page")
            sleep(0.1)
            continue
        except NoSuchElementException:
            log("Element does not exist when trying to click!")
            return

def java_click(ele, variance):
...
def skill_check():
    ...
    for counter in range(0, len(cards)):
        ...
        for charcount in range(0, len(char)):
            if char[charcount].get_attribute("class") == "lis-ability btn-ability-available quick-button":
...
def do_skill(character, skill, target=-1):
    ...
    log("Using character %s's skill %s. Target %s" %(character, skill, target))
    actions = ActionChains(GBF)
...
def do_attack(auto=True, ougi=False):
    ...
    clicker(".btn-attack-start.display-on")
    if auto:
        while not ele_check(".btn-auto"):
            sleep(0.1)
        clicker(".btn-auto")
        return
...
def claim_rewards():
    if ele_check(".btn-usual-ok") is True:
...
def results_page(homepage, target, rounds):
    log("In results page")
    wait_until_css(".btn-usual-ok", maxwait=0)
...
def create_coop_lobby():
    ...
    wait_until_css(target, maxwait=0)
    ele = GBF.find_element(By.CSS_SELECTOR, target)
    clicker(ele)
    wait_until_css(target2, maxwait=0)
    ele = GBF.find_element(By.CSS_SELECTOR, target2)
    clicker(ele)

def coop_lobby():
    """We don't handle potting yet LOL"""
    log("In coop lobby page")
    if GBF.current_url == "http://game.granbluefantasy.jp/#coopraid":
        create_coop_lobby()
    target = ".btn-repeat-last"
    target2 = ".btn-quest-start.multi.se-quest-start"
    if ele_check(".btn-make-ready-large.not-ready"):
...
def main():
    ...
    page = "http://game.granbluefantasy.jp/#mypage"
    target = ".btn-link-party.se-ok"
    load_page(page, target)
    input("Press enter to continue")
    main_loop()
